...
def turn(angle, dontTurn):
    ...
    if (angle < -25):
        ...
        leftSpeed = STRAIGHT_SPEED - min((abs(angle)/45) * 0.2, 0.2)
        rightSpeed = STRAIGHT_SPEED - min((abs(angle)/45) * 0.2, 0.2)
...
def bang_bang_steering(frame, bbox, angle, object_area):
    if(not PI):
        return
    ...
    if x_center < frame_center:
        turn(angle + 20 * object_area/200,000, False)
    ...
    while cap.isOpened():
        ...
        if (frame_counter % steering_rate == 0):
            ...
            if (object_area >min_area):
                if(lane_lines_yellow > 0 and lane_lines_blue == 0):
                    print("just yellow")
                    turn(previous_angle + `45`*object_area/area_scaler)
                elif(lane_lines_blue > 0 and lane_lines_yellow == 0):
                    turn(previous_angle - 45*object_area/area_scaler)
                    print("just blue, angle now", previous_angle - 45*object_area/200,000)
                else:
                    # is_colliding(frame, object_purple)):
                    # TODO: Tune bang-bang steering speed/angle
                    print("both lanes")
                    bang_bang_steering(frame, object_purple, previous_angle, object_area)
                continue
